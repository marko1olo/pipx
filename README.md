<div align="center">
  <img src="docs/assets/banner.png" alt="pipx Banner" width="100%" />
  ...
  **Install and Run Python Applications in Isolated Environments.**
  ...
  [![PyPA](https://img.shields.io/badge/PyPA-Packaging-blue?style=flat&logo=pypa&logoColor=white)](https://pypa.io)
  [![Build](https://img.shields.io/badge/Build-Hatch-8A2BE2?style=flat)](https://hatch.pypa.io/)
  [![Environment](https://img.shields.io/badge/Environment-Virtualenv-0055E6?style=flat)](https://virtualenv.pypa.io/)
  [![License](https://img.shields.io/badge/License-MIT-green.svg?style=flat)](https://github.com/pypa/pipx/blob/main/LICENSE)
</div>
...
```mermaid
flowchart TD
    CLI[User Terminal Command / CLI Arguments] --> Main[pipx.main CLI Parser]
    Main --> Specifier[Package Specifier Parser & Resolution]
    Specifier --> VenvMgr[pipx.venv Venv Manager]
    
    VenvMgr --> IsolatedVenv[(Isolated ~/.local/pipx/venvs/<app>)]
    VenvMgr --> SharedLibs[pipx.shared_libs Shared Wheels]
    
    IsolatedVenv --> PipInstall[Execute pip install]
    PipInstall --> BinSymlink[Symlink Executables to ~/.local/bin]
    
    BinSymlink --> UserPath[User System PATH Available Commands]
```

### Component Matrix

| Module Path | Core Purpose & Architecture Role | Key Responsibilities |
|---|---|---|
| `src/pipx/main.py` | Command Line Interface entry point and subcommand dispatcher | Argument parsing, CLI command execution (`install`, `run`, `inject`, `list`) |
| `src/pipx/venv.py` | Virtual environment management and pip invocation layer | Creation of isolated venvs, package installation, wheel caching |
| `src/pipx/package_specifier.py` | Requirement specifier and URL parser | Parsing PyPI names, VCS URLs, local paths, and spec constraints |
| `src/pipx/paths.py` | Path resolution and environment context manager | Resolving `PIPX_HOME`, `PIPX_BIN_DIR`, and global paths |
| `src/pipx/shared_libs.py` | Shared wheel and core dependency cache manager | Managing shared pip, setuptools, and wheel packages across venvs |
| `src/pipx/standalone_python.py` | Standalone Python interpreter downloader and management | Fetching and bootstrapping standalone Python binaries |
| `src/pipx/pipx_metadata_file.py` | Metadata serialization for installed applications | Maintaining `pipx_metadata.json` state inside virtual environments |
...
### Обзор проекта pipx
**pipx** — это официальный инструмент экосистемы **PyPA** (Python Packaging Authority), предназначенный для установки и запуска консольных Python-приложений в изолированных виртуальных окружениях. В отличие от стандартного `pip install --user`, `pipx` предотвращает конфликты зависимостей между различными CLI-утилитами (например, `black`, `flake8`, `ansible`, `yt-dlp`, `poetry`, `mypy`).

### Ключевые возможности
1. **Изолированная установка (`pipx install`)**: Каждое приложение устанавливается в свое собственное изолированное виртуальное окружение (`~/.local/pipx/venvs/<app>`), а исполняемые бинарные файлы симлинкаются в общую системную папку (`~/.local/bin`).
2. **Одноразовый запуск (`pipx run`)**: Запуск любой утилиты без предварительной постоянной установки во временном окружении с автоматическим кешированием.
3. **Управление зависимостями (`pipx inject`)**: Добавление дополнительных библиотек и плагинов в уже существующее окружение установленного приложения.
4. **Централизованное обновление (`pipx upgrade-all`)**: Безопасное обновление всех установленных CLI-инструментов одной командой.

### Установка и настройка
```bash
### Установка через pip / brew / apt
pip install pipx
pipx ensurepath
```
